function [ERP, time] = calc_ERP_perRat(Data,cfg,useClean, bootStrap)
...
allErp_rat = zeros(nCh,smple_num,nRat);
stdErp_rat = zeros(nCh,smple_num,nRat);
targetErp_rat = zeros(nCh,smple_num,nRat);
changedErp_rat = zeros(nCh,smple_num,nRat);
unChangedErp_rat = zeros(nCh,smple_num,nRat);
...
for iRat=1:nRat
    ...
    for iSess=1:nSess
        ...
        if(~bootStrap)
            allErp = mean(DataSess(:,:,allTr),3,'omitnan');
            stdErp = mean(DataSess(:,:,stdTr),3,'omitnan');
            targetErp = mean(DataSess(:,:,targetTr),3,'omitnan');
            changedErp = mean(DataSess(:,:,changedTr),3,'omitnan');
            unChangedErp = mean(DataSess(:,:,unChangedTr),3,'omitnan');
        else
