...
nn = sum(~isnan(amatrix),1);
astd = nanstd(amatrix,[],1)/sqrt(nn(1)); % to get sem shading
...
if exist('alpha','var')==0 || isempty(alpha) 
    ...
    fillOut = fill([F fliplr(F)],[amean+astd fliplr(amean-astd)],acolor, 'FaceAlpha', alpha, ...
...
hold on;
lineOut = plot(F,amean,'color', acolor,'linewidth',lw); %% change color or linewidth to adjust mean line
if check
    hold off;
end
end
