...
for iCh=1:4
    ...
    ax(2) = nexttile;
    img = squeeze(mean(after10_pac_rat{iState,1}(ch1,ch2,ti,famp_idx,fph_idx,iRat,iSess),[3 6 7], 'omitnan'));
    pcolor(fph_mesh,famp_mesh,img); shading interp; colormap jet
    clim([0 c_max])
    title('After10', 'FontSize', 6);
    
    ax(3) = nexttile;
    img = squeeze(mean(after20_pac_rat{iState,1}(ch1,ch2,ti,famp_idx,fph_idx,iRat,iSess),[3 6 7], 'omitnan'));
    pcolor(fph_mesh,famp_mesh,img); shading interp; colormap jet
    clim([0 c_max])
    title('After20', 'FontSize', 6);
    ...
    fig_dir = fullfile('../', "Figures","PACcomo_trialSwap_oddball_allD_"+ch_names(iCh)+".pdf");
...
cb.Label.Rotation = 0;  % Rotate label if desired
...
fig_dir = fullfile('../', "Figures","PACcomo_trialSwap_oddball_allD_colorbarH.pdf");
exportgraphics(gcf, fig_dir, 'ContentType', 'vector', 'Resolution', 300);
...
hLine = line([600 600], yl, 'LineWidth', lw, 'Color', [0.5 0.5 0.5], 'LineStyle', '-.', 'handleVisibility', 'off');
uistack(hLine, 'bottom');  % push behind other plots
text(500, 3.5e-4, '***','FontSize',14, 'FontWeight', 'bold')
